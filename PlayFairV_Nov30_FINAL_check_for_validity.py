...
keys=[]
with open("third_filter.txt",'r') as f:
	lines=f.readlines()
	for line in lines:
		# print(type(line))
		# print(line)
...
for i in keys:
	...
	text=decrypt(i,msg)
	str_text="".join(text)
	if(str_text.find("that")!=-1):
		if(str_text.find("for")!=-1):
			if(str_text.find("whose")!=-1):
				if(str_text.find("action")!=-1):
					if(str_text.find("changes")!=-1):
						print(i)
						print("=====")
						print(text)
